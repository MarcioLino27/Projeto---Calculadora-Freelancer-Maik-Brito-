  ...
  <div class="max-w-4xl">
    <section id="top" class="animate-up flex justify-between items-center border-b border-b-gray-600 pb-8">
      ...
      <span id="notification" class="flex gap-2">
        ...
        {:else }
          Você tem {freeHours} {freeHours == 1? 'hora livre' : 'horas livres'} livres no seu dia
        {/if}
      </span>
      <button 
      id="avatar-profile"
      class='flex items-center gap-4 hover:text-orange-400 transition'>
        <div class="grid text-end">
          ...
          <span class='text-xm'>Ver perfil</span>
        </div>
        <img 
          class="w-14 border-2 border-orange-400 rounded-full"
        src={profile.avatar} alt="Avatar de perfil de {profile.name}" />
      </button>
    </section>
    ...
    <section id="summary" class="animate-up delay-1 flex justify-between items-center">
      ...
      <button class="button orange uppercase grow-0 flex gap-4 bg-orange-400 h-fit px-3 py-2 rounded items-center hover:brightness-110 transition-all">
