...
<div
  class={`border border-gray-200 grid grid-cols-[35%_20%_15%_20%_10%] items-center py-6 px-8 rounded bg-white hover:bg-gradient-to-l hover:from-transparent  hover:to-orange-50 overflow-hidden relative before:absolute before:top-0 before:left-0 before:w-1 before:h-0 before:transition-all before:bg-orange-400 hover:before:h-full`}>
  ...
  <div class="actions column flex gap-2">
    ...
    <button
      on:click={goToProject} class="border border-gray-200 p-2 rounded hover:bg-gray-100 transition-all" title="Editar Job"
    >
      <img class="w-5" src="./images/edit-24.svg" alt="Editar Job" />
    </button>
    <button
      on:click={handleDelete} class="border border-gray-200 p-2 rounded hover:bg-red-100 transition-all" title="Excluir Job"
    >
